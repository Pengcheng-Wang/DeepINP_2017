...
function Setup:parseOptions(arg)
  ...
  cmd:option('-actSmpEps', 0.35, 'User action sampling threshold. If rand se than this value, reture 1st pred')
  cmd:option('-termActSmgLen', 60, 'The lenght above which user termination action would be highly probably sampled')
  cmd:option('-termActSmgEps', 0.9, 'The probability which user termination action would be sampled after certain length')
  cmd:option('-rwdSmpEps', 0.65, 'User rwd sampling threshold. If rand se than this value, reture 1st pred')
